...
export function useRecord(rec, mdeInstance) {
  ...
  const mdIn = String(rec?.body_md ?? rec?.bodyMd ?? rec?.Body_md ?? rec?.Body ?? "");
...
function queueMdeBackfill(rec) {
  ...
  __mdeBackfillTimer = window.setInterval(() => {
    ...
    if (mde && typeof mde.value === "function") {
      let mdIn = String(rec?.body_md ?? rec?.bodyMd ?? rec?.Body_md ?? rec?.Body ?? "");
      if (!mdIn && (rec?.body_html || rec?.bodyHtml)) {
        // 아주 단순 폴백: HTML을 텍스트/간이 마크다운으로 변환
        const h = String(rec.body_html || rec.bodyHtml || "");
        mdIn = h
          .replace(/<br\s*\/?>/gi, "\n")
          .replace(/<\/p>/gi, "\n\n")
          .replace(/<h([1-6])[^>]*>([\s\S]*?)<\/h\1>/gi, (_, n, t) => "#".repeat(+n) + " " + t.trim())
